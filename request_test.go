...
func TestTargetGeneration(t *testing.T) {
	...
	one := targets[5]
	Equal(t, one.URL, "https://example.com/FUZZjohnSTR/12345?id=abc&city=Paris")
	EqualBytes(t, one.Body, []byte(`{"age": 35, "date": "2024-09-08"}`))

	two := targets[2]
	Equal(t, two.URL, "https://example.com/john/FUZZ12345UID?id=abc&city=Paris")
	EqualBytes(t, two.Body, []byte(`{"age": 35, "date": "2024-09-08"}`))

	three := targets[4]
	Equal(t, three.URL, "https://example.com/john/12345?id=FUZZabcSTR&city=Paris")
	EqualBytes(t, three.Body, []byte(`{"age": 35, "date": "2024-09-08"}`))

	four := targets[1]
	Equal(t, four.URL, "https://example.com/john/12345?id=abc&city=FUZZParisSTR")
	EqualBytes(t, four.Body, []byte(`{"age": 35, "date": "2024-09-08"}`))

	five := targets[3]
	Equal(t, five.URL, "https://example.com/john/12345?id=abc&city=Paris")
	EqualBytes(t, five.Body, []byte(`{"age": FUZZ35NUM, "date": "2024-09-08"}`))

	six := targets[0]
	Equal(t, six.URL, "https://example.com/john/12345?id=abc&city=Paris")
